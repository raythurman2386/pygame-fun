...
class Menu(tools._State):
    ...
    def startup(self, *args):
        ...
        self.level_surface = pygame.Surface(self.map_rect.size)
        ...
        self.alpha = 255
        self.transition_surface = pygame.Surface(setup.SCREEN_RECT.size)
...
class LoadGame(Instructions):
    ...
    def normal_update(self, keys):
        if self.allow_input:
            if keys[pygame.K_DOWN] and self.arrow.index == 0:
                self.arrow.index = 1
                self.notify(c.CLICK)
                self.allow_input = False
            elif keys[pygame.K_UP] and self.arrow.index == 1:
                self.arrow.index = 0
                self.notify(c.CLICK)
                self.allow_input = False
